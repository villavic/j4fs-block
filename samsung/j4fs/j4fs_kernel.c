...
// Fix as per http://lwn.net/Articles/428783/
				...
				struct page **pagep, void **fsdata)
{
	...
	if(to>PAGE_CACHE_SIZE) {
		T(J4FS_TRACE_ALWAYS,("%s %d: page size overflow(pos,index,offset,len,to)=(%d,%d,%d,%d,%d)\n",__FUNCTION__,__LINE__,(unsigned int)pos,(unsigned int)index,(unsigned int)offset,(unsigned int)len,to));
				...
				struct page *pg, void *fsdadata)
{
	...
	T(J4FS_TRACE_FS,
		("j4fs_write_end addr %#lx pos %#lx nBytes %d\n",
		(unsigned long int)addr,
		(unsigned long int)pos, copied));
...
// J4FS for Block Devices fron NiTRo
#if defined(J4FS_USE_BLK)
void j4fs_unmount (struct super_block * sb)
{
	FlashDevUnmount();
...
MODULE_AUTHOR("Samsung Electronics Co.Ltd. 2009-2010 All Right Reserved");
MODULE_DESCRIPTION("Jong Jang Jintae Jongmin Filesystemi (J4FS)");
MODULE_LICENSE("GPL");
